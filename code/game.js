...
var runWobbleSpeed = 50, runWobbleDist = 0.07;

Runner.prototype.act = function(step) {
     this.wobble += step * runWobbleSpeed;
     var runWobblePos = Math.sin(this.wobble) * runWobbleDist;
     this.pos = this.basePos.plus(new Vector(0, runWobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" || type == "hazard" && this.status == null) {
    ...
    if (!this.actors.some(function(actor) {
           return actor.type == "coin" || actor.type == "runner";
         }))
         {
      this.status = "won";
      this.finishDelay = 1;
